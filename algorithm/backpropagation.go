...
func fit(X [][]float64, D [750]float64, Epochs int) (Errors [1000]float64, o_weight [6]float64, o_bias [3]float64) {
	rows := len(X)
	...
	for Epoch < Epochs {
		...
		for j < rows {
			...
			Weight[2] = Weight[2] + (delta_h2 * X[j][0] * Learning_Factor)
...
func prediction(weights [6]float64, bias [3]float64, v_age, v_gender float64) (out_y float64) {
	//-----------------------Propagation--------------------------
	//We calculate the total net input of the hidden layer
	net_h1 := (weights[0] * v_age) + (weights[1] * v_gender) + bias[0]
	net_h2 := (weights[2] * v_age) + (weights[3] * v_gender) + bias[1]
	...
	//We calculate the total net output of the output layer
	net_y := (weights[4] * out_h1) + (weights[5] * out_h2) + bias[2]
...
func httpserver(w http.ResponseWriter, _ *http.Request) {
	...
	out_y := prediction(weights, bias, v_age, v_gender)
	fmt.Println(out_y)
}
